...
# 分批次处理的函数
def batch_process(model, syn_imgs, reconstructed_ref, batch_size):
    ...
    num_images = len(syn_imgs)
    pixel_diff_list = []
    feature_diff_list = []

    for i in range(0, num_images, batch_size):
        # 获取当前批次的图像
        batch_syn_imgs = syn_imgs[i:min(i + batch_size, num_images)]
        batch_ref_imgs = reconstructed_ref[i:min(i + batch_size, num_images)]

        # 计算像素差异
        batch_pixel_diff = CR(batch_syn_imgs, batch_ref_imgs).sum(dim=1, keepdim=True)
        pixel_diff_list.append(batch_pixel_diff)

        # 计算特征差异
        model_output_ref = model(batch_syn_imgs)
        model_output_syn = model(batch_ref_imgs)
        ...
        # weights = [0.2, 0.6, 0.2]
        weights = [1, 1, 1]
        batch_feature_diff = sum([
            weights[j] * upscale(CR(model_output_ref[j], model_output_syn[j]).sum(dim=1, keepdim=True))
            for j in range(len(model_output_ref))
        ])
        feature_diff_list.append(batch_feature_diff)

    # 合并所有批次的结果
    pixel_diff = torch.cat(pixel_diff_list, dim=0)
    feature_diff = torch.cat(feature_diff_list, dim=0)

    return pixel_diff, feature_diff
...
def evaluate_anomaly_detection(syn_imgs, ref_imgs, gt_masks, all_labels, args):
    ...
    with torch.no_grad():
        ...
        # feature_diff = sum([
        #     upscale(CR(model(reconstructed_ref)[i], model(syn_tensors)[i]).sum(dim=1, keepdim=True))
        #     for i in range(len(model(reconstructed_ref)))
        # ])
        
        pixel_diff, feature_diff = batch_process(model, syn_tensors, reconstructed_ref, batch_size=16)
...
def visualize_results(syn_imgs, ref_imgs, scores, result_dir, args, version):
    """生成可视化对比图"""
    ...
    for idx in tqdm(range(len(syn_imgs)), desc="Generating visualizations"):
        ...
        if not os.path.exists(f"./Draw/{version}/{args.classname}/"):
            os.mkdir(f"./Draw/{version}/{args.classname}/")
        overlay_img_pil = Image.fromarray((overlay_img * 255).astype(np.uint8))  # 转换为 8-bit 图像
        overlay_img_pil.save(f'./Draw/{version}/{args.classname}/{idx}.png')  # 保存图像到指定路径
...
def main():
    ...
    save, visual = True, True
    # 保存和可视化
    if save:
        save_results(args, metrics, result_dir)
    if visual:
        version = "L3"
        visualize_results(syn_imgs, ref_imgs, scores, gt_masks, args, version)
